        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-10 relative z-10">
          <div className="space-y-4">
            ...
            <p className="max-w-xl text-slate-400 font-medium leading-relaxed">
              Your expert feedback directly influences the ensemble weighting. Correcting a single
              false result refines the entire decision perimeter for the next iteration.
            </p>
          </div>

          <div className="text-center p-8 glass-card border-white/5 rounded-3xl min-w-[200px]">
            <p className="text-5xl font-black text-indigo-400 glow-text-indigo leading-none mb-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                <Search
                  size={16}
                  className="absolute left-4 top-3.5 text-slate-600 group-focus-within:text-indigo-400 transition-colors"
                />
                <input
                  type="text"
                  required
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl flex-center gap-3 font-black text-xs uppercase tracking-[0.2em] transition-all shadow-lg ${
                success
                  ...
                  : isSubmitting
                    ? 'bg-indigo-600/40 text-slate-300'
                    : 'bg-indigo-600 text-white hover:bg-indigo-500 shadow-indigo-900/40'
              }`}
            >
              {success ? (
                <>
                  <CheckSquare size={16} /> Signal Transmitted
                </>
              ) : (
                <>
                  <Send size={16} /> Re-calibrate Engine
                </>
              )}
            </button>
          </form>
          ...
          <div className="p-10 glass-card rounded-[2.5rem] border-white/5 flex-1 relative overflow-hidden flex flex-col justify-center">
            ...
            <ul className="space-y-5 text-sm font-medium text-slate-400">
              <li className="flex items-start gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-indigo-500/20 text-indigo-400 flex-center font-black text-[10px]">
                  1
                </span>
                <span>
                  The scoring matrix adjusts its weightings for similar behavioral signatures.
                </span>
              </li>
              <li className="flex items-start gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-indigo-500/20 text-indigo-400 flex-center font-black text-[10px]">
                ...
                <span>
                  Verified exceptions are cached to prevent future friction for high-performing
                  providers.
                </span>
              </li>
              <li className="flex items-start gap-3">
                ...
                <span>
                  Natural Language is parsed to extract new rule-base primitives automatically.
                </span>
              </li>
            </ul>
          </div>
          ...
          <div className="p-8 glass-card rounded-[2rem] border-emerald-500/10 bg-emerald-500/5 flex items-center gap-6 group">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-slate-300">
                Retraining cycle running in background. High priority feedback applied.
              </p>
            </div>
          </div>
        </div>
