...
const KPICard = ({ title, value, change, icon: Icon, color, delay }) => (
  ...
  >
    <div
      className="absolute -right-4 -top-4 w-24 h-24 bg-current opacity-[0.03] rounded-full blur-2xl group-hover:scale-150 transition-transform duration-700"
      style={{ color }}
    ></div>
    ...
    <div className="flex justify-between items-start mb-6">
      ...
      <div
        className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-lg bg-white/5 border border-white/10 ${change.startsWith('+') ? 'text-emerald-400' : 'text-rose-400'}`}
      >
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <KPICard
          ...
          value={kpis?.totalAnalyzed?.toLocaleString() || '15,420'}
        ...
        <KPICard
          ...
          value={kpis?.underInvestigation || '85'}
          change="12 assigned today"
          icon={Users}
          color="#f59e0b"
          delay={0.3}
        />
        <KPICard
          ...
          value={`$${(kpis?.totalPrevented / 1000).toFixed(0)}k`}
          change="+$142k this week"
          icon={ShieldCheck}
          color="#10b981"
          delay={0.4}
        />
          ...
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-1">
                Anomaly Scoring Volume (Last 7 Days)
              </p>
            </div>
            <div className="flex gap-2">
              <span className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex-center text-slate-400 hover:text-white transition-colors cursor-pointer">
                <Activity size={16} />
              </span>
            </div>
          </div>
        ...
        <div className="glass-card p-8 rounded-[2rem] border-white/5 flex flex-col justify-center">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                Auto-detected behavioral markers identified today.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                key={idx}
                className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5 hover:border-white/10 transition-all group"
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-[10px] font-black px-1.5 py-0.5 rounded ${sig.drift === 'Safe' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}
                  >
                    {sig.drift}
                  </span>
                </div>
