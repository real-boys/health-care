  ...
  const [rules, setRules] = useState([
    {
      ...
      min: 0,
      max: 500,
      types: ['PreventiveCare'],
      autoApprove: true,
      active: true,
    },
    {
      id: 2,
      name: 'Standard Singuery Filter',
      min: 1000,
      max: 5000,
      types: ['Surgery'],
      autoApprove: false,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {rules.map(rule => (
          <div
            key={rule.id}
            className="bg-white p-6 rounded-2xl border-2 border-gray-100 hover:border-blue-200 transition group"
          >
            <div className="flex justify-between items-start mb-4">
              <div
                className={`p-3 rounded-xl ${rule.autoApprove ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'}`}
              >
                {rule.autoApprove ? (
            ...
            <div className="space-y-3 text-sm text-gray-600">
              ...
              <div className="flex justify-between items-center">
                ...
                <div
                  className={`w-10 h-5 rounded-full relative transition-colors ${rule.autoApprove ? 'bg-green-500' : 'bg-gray-300'}`}
                >
                  <div
                    className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${rule.autoApprove ? 'right-1' : 'left-1'}`}
                  />
                </div>
              </div>
            </div>
        ...
        <div className="relative z-10 flex justify-between items-end">
          <div>
            ...
            <p className="text-gray-400 max-w-md">
              Automated systems have processed 84% of claims this month with zero manual
              intervention needed.
            </p>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-2xl border shadow-sm">
          <h4 className="font-bold mb-6 flex items-center">
            <FileText className="w-5 h-5 mr-3 text-blue-600" /> Volume by Type
          </h4>
          <div className="space-y-6">
            {[
        ...
        <div className="bg-white p-8 rounded-2xl border shadow-sm">
          <h4 className="font-bold mb-6 flex items-center">
            <BadgeAlert className="w-5 h-5 mr-3 text-orange-600" /> Top Exceptions
          </h4>
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex justify-between items-center p-4 bg-gray-50 rounded-xl">
                ...
                <span className="bg-white px-3 py-1 border rounded-lg font-bold text-orange-600">
                  {e.cases} cases
                </span>
              </div>
            ))}
