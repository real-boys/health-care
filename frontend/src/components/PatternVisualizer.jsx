    ...
    <div className="relative w-full h-[500px] glass-card rounded-[3rem] border-white/5 overflow-hidden group">
      <div className="absolute top-8 left-8 z-10">
        ...
        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">
          Provider-Patient Cluster Analysis
        </p>
      </div>
      ...
      <div className="absolute bottom-8 left-8 right-8 z-10 flex justify-between items-end">
        ...
        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] animate-pulse">
          Scanning Relational Graph...
        </p>
      </div>
    </div>
  );
...
const HeatmapCell = ({ value, delay }) => {
  ...
  const getCellColor = val => {
    if (val > 0.8)
      return 'bg-rose-500 shadow-[inset_0_0_10px_rgba(0,0,0,0.3)] shadow-rose-900/40 border-rose-400/30';
            ...
            <div className="flex-1 space-y-2">
              <div className="grid grid-cols-7 gap-2">
                {days.map(d => (
                  <span
                    key={d}
                    className="text-[10px] font-black text-slate-600 uppercase text-center mb-1"
                  >
                    {d}
                  </span>
                ))}
            ...
            <div className="flex-1 h-1.5 mx-4 flex gap-1">
              {[...Array(6)].map((_, i) => (
                <div
                  ...
                  style={{
                    backgroundColor: i < 2 ? '#1e293b' : i < 4 ? '#6366f1' : '#ef4444',
                    opacity: 0.1 + i * 0.2,
                  }}
                ></div>
              ))}
        ...
        <div className="glass-card p-8 rounded-[2.5rem] border-white/5 relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              {[
                {
                  label: 'Impossible Distance Spike',
                  val: 'Critical',
                  icon: Globe,
                  color: 'text-rose-500',
                },
                {
                  label: 'Provider-Patient Loop',
                  val: 'Detected',
                  icon: Fingerprint,
                  color: 'text-indigo-400',
                },
                {
                  label: 'Night-Shift Billing Cluster',
                  val: 'Warning',
                  icon: Layers,
                  color: 'text-amber-500',
                },
                {
                  label: 'UPC Mismatch Signature',
                  val: 'Confirmed',
                  icon: Zap,
                  color: 'text-rose-400',
                },
                ...
                <div
                  key={i}
                  className="flex items-center gap-4 p-4 rounded-3xl bg-white/5 border border-white/5 hover:border-white/10 transition-all group"
                >
