  ...
  return (
    <Link to={to} className="relative group block outline-none">
      ...
      >
        ...
        {badge && (
          <span
            className={`ml-auto px-2 py-0.5 rounded-lg text-[10px] font-black ${
      ...
      <aside className="w-72 bg-[#050608] border-r border-slate-900 flex flex-col hidden xl:flex shrink-0 h-screen sticky top-0 z-50">
        <div className="p-8">
          <div className="flex items-center gap-3 mb-10">
            ...
            <div>
              <span className="block font-black text-xl text-white tracking-tighter leading-tight">
                AEGIS
              </span>
          ...
          <nav className="space-y-1.5">
            <p className="text-[10px] font-bold text-slate-600 uppercase tracking-[0.15em] mb-4 ml-4">
              Command Center
            </p>
            ...
            <div className="pt-4">
              ...
              />
              <SidebarItem to="/i18n" icon={<Globe size={20} />} label="Multi-language" />
              <SidebarItem
                to="/accessibility"
                icon={<Accessibility size={20} />}
                label="Accessibility"
              />
              <SidebarItem to="/onboarding" icon={<BookOpen size={20} />} label="Onboarding" />
            </div>
          </nav>
        </div>
...
const Placeholder = ({ name }) => (
  <div className="flex-center flex-col min-h-screen text-slate-500 space-y-4">
    ...
    <p className="max-w-xs text-center text-sm">
      This module is currently initializing. Please check Fraud Intelligence for a live
      demonstration.
    </p>
  </div>
);
      ...
      <Router>
        <Layout>
          <Routes>
            ...
            <Route path="/analytics" element={<InteractiveDataVisualization />} />
            <Route path="/i18n" element={<LanguageSwitcher />} />
            <Route path="/accessibility" element={<AccessibilityCompliance />} />
