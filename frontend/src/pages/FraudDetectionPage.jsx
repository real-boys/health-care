...
export const FraudDetectionPage = () => {
  ...
  const updateCase = async (id, update) => {
    setStats(prev => ({
      ...prev,
      cases: prev.cases.map(c => (c.id === id ? { ...c, ...update } : c)),
    }));
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-12">
            ...
            >
              ...
              <p className="text-slate-400 font-medium max-w-xl">
                Real-time ensemble forensics powered by Isolation Forest & behavioral pattern
                matching.
              </p>
            </motion.div>
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              ...
              {activeTab === 'cases' && (
                <CaseManager cases={stats.cases} onUpdateCase={updateCase} />
              )}
              {activeTab === 'alerts' && <AlertsPanel alerts={stats.alerts} />}
              {activeTab === 'patterns' && (
                <PatternVisualizer
                  networkData={stats.networkData}
              ...
              {activeTab === 'feedback' && (
                <FeedbackSystem
                  submitFeedback={async (cid, r) => {
                    await axios.post('http://localhost:3000/api/fraud/feedback', {
                      claimId: cid,
                      reason: r,
                    });
